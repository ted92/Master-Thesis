...
%TODO: talk about the Rizun paper, how we tested the optimal block size limit and discuss the causes that might bring to an increase or not to a block size.
...
or not the block size limit are still ongoing and this issue has been discussed
...
following pros ($+$) and cons ($-$)\,\cite{blocksizecontroversy}:%TODO: RELATION WITH FEE DENSITY AND FEE PAID (TWO AREA GRAPHS AND Q GRAPH)
\begin{itemize}
...
%TODO: Check the transaction size during time
...
$\sim$$3$-$4$\,txs/sec. If compared with centralized trust-oriented
