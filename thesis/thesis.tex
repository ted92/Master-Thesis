...
stored in our own data frame, saving up to $x10$ space
for the analysis.
...
%TODO?: Ask questions like: "Is it possible to pay for bandwidth in Bitcoin?" or "How much fee influence the transaction latency?" or "Do any major mining pools systematically exclude zero-fee transactions?" or "Will Bitcoin blockchain manage the constant increment of transactions to approve per day?"

%TODO: Sentences:
% You can't build something that works without having a certain level of centralization, there always should be a compromise between centralization and how much you can trust spontaneous orders to take over.
%\begin{quote}
%\emph
...
%TODO: Talk briefly about mining
\section{Mining}
\label{sec:mining}
In a decentralized cryptocurrency network transactions need to be approved by all the
participant before being validated and confirmed. This validation requires high computational
time and it might be expensive in matter of electricity and resources used.
...
create new blocks by solving a proof-of-work
...
is often referred as mining\,\cite{bitcoinmining_process}.
...
A miner choose whether include a transaction into a block or not,
...
of the fees offered by each transaction included in the block,
$M$, plus the block reward, $R$, which
is now set at $12.5$\,\bitcoin.
After the new block is mined, this information
is spread through the whole network and
if this solution is propagated to the $50\% + 1$
of the nodes in the system then
the block is validate and all the
transactions in it are accepted and confirmed.
Finally, miners in the network express their acceptance by
moving to work on the next block,
incorporating the hash of the accepted block.

\subsection{Proof-of-Work}
\label{sec:proof-of-work}
Cynthia Dwork and Moni Naor defined the proof-of-work as follows\,\cite{proof-of-work}:
...
transactions, more miners means a more
secure network.
To become a Bitcoin miner nowadays you
need to buy highly specified chips called
\gls{asic}. Informations about this
hardware can be found at
\url{bitcoinmining.com}\,\cite{bitcoinmining}.
In early days of Bitcoin ($2009$-$2013$)
it was possible to mine with your computer CPU
or high speed video processor card.
Today that's no longer possible since custom
Bitcoin \gls{asic} chips offer performance
up to $x100$ the capability of older
systems\,\cite{bitcoinmining} and another reason is
the coming of mining pools, bringing then a constant
growth of the \emph{mining difficulty}.

\definition{A \emph{pooled mining} combines the work of many miners
	toward a common goal.}

Pools of miners or mining pools
find solutions faster than individual member and each
...
while keeping the Bitcoin network \emph{stable}, \emph{safe} and \emph{secure}\,\cite{bitcoinmining}.
...
%TODO: Explain how throughput, fee density and other derived data are calculated
...
are represented and discussed. We state that %TODO: FINISH STATEMENT ACCORDING TO RESULTS
...
Scalability may concern the number of nodes in the network but also
the amount of transaction's requests per second that the system faces.
The constant growth of the cryptocurrencies' popularity rises not few
concerns regarding the scalability of the system. The reward for miners
brought a lot of people being interested in mining, and
the number of nodes for this purpose highly increased
during years.%TODO: COUNT NUMBER OF MINERS
This brought the emerge of \emph{mining pools}, which consists in having
a lot of miners connected together to find as fast as possible
the solution of the puzzle and share the earned reward afterwards.
...
to join the network. %TODO: Show that in 2013 there were more independent node while nowadays most of the blocks mined are covered from major minin pools.

The other scalability problem is related to the growing amount
of transactions that need to be approved every day. It is obvious that
with an average $t_q$ of $500$\,bytes, a fixed estimated
$\mathcal{T}$$=$$10$ minutes
and a maximum block size $Q$ fixed at $1$\,MB, the outside number
of transactions the system can approve per second is roughly $3$-$4$ txs/sec.
Compared to systems like VISA that can
easily approve $2000$ transactions per second, Bitcoin
...
Master Card. However we want to analyze this boundaries and understand
what it possible to do according to improve the scalability.
...
\begin{figure}[h]
	...
	\caption{Relation between $t_l$ and $Q$ from $2014$ to $2017$.}
	\label{fig:blocksize_latency}
\end{figure}

Not much intuitive and more difficult to study was also the
relation between $t_l$ fee and $t_f$. According
to analysis made on $t_f$ we opportunely categorized the fee paid
...
more than $100$\,million of transactions from $2013$ to $2017$.
\begin{table}
	\centering
	...
	\begin{tabular}{|p{2.5cm}|p{4.5cm}|p{3cm}|} \hline
		\textbf{Fee category}&\textbf{$t_f$ value (\bitcoin)}& \textbf{$t_l$ variation (h)}\\
		\hline
		$0$&$t_f = 0$& $1$ to $33$\\
		\hline
...
mid $2016$ we had a big change in the way miners include transactions.
If in $2013$-$2014$ miners and mining pools tended to include in blocks
...
%TODO: Explain that with another graph - regression ones
...
\end{figure}
%TODO: Fee density and fee are directly connected, since the transaction size $t_q$ had always an average of $500$\,bytes. At the end of $2017$ eve if we have some fees with 0\bitcoin, the fee density is higher than 0, which means that recently, miners might have changed from constraints on fees to constraints on fee density.
\begin{figure}[h]
	\centering
...
\cite{linearize}, \cite{Tedeschi:2016:PBB}, \cite{Donnelly2015BNSB}, \cite{houy2014EOBTF},
